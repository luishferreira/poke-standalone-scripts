...
### `hunt-recommender.user.js`

- Userscript próprio de análise, executado em `document-start` e exposto em `window.piwHuntRecommender`.
- Usa o bridge para solicitar/receber `pokes` e eventos. A análise não altera a hunt nem envia ações de combate; somente o botão manual de cada linha pode trocar de hunt.
- Cruza o líder equipado e `/api/characters/me` com `/game/creatures.json` e `/api/game/map-markers`; o nível do treinador sempre limita hunts desbloqueadas. O nível do Pokémon não limita o ranking por padrão para permitir power leveling, mas um checkbox persistido por aba pode excluir hunts acima dele.
...
- O botão de entrada usa o marcador visual real (`data-guide="hunt-<slug>"`), percorre as áreas do mapa e aguarda o `enter-hunt` emitido pelo jogo, como o Auto Pokédex. Nunca envie `enter-hunt` diretamente. Bloqueie a troca quando Auto Pokédex ou Auto Boss estiverem ativos.
...
- Registra o botão no menu lateral compartilhado e persiste a posição do painel e a preferência de limite por nível no `sessionStorage`.
...
- Todas as configurações, inclusive produtos e itens protegidos, só são editáveis com o Auto Refill pausado. A interface tem resumo compacto, tela de configurações e painel lateral de itens protegidos aberto a partir dela. Pausar e ativar novamente rearma as categorias; não há botão de rearme na UI. A troca de produto passa estoque, label e compra para o novo ID, mas não altera qual potion ou ball o bot oficial do jogo está configurado para consumir.
...
- Os presets de Stones e Shiny apenas materializam seus IDs em `protectedItemIds`; não executam locks ao serem aplicados. As listas expansíveis permitem remover exceções individuais. Stones vêm das entries de `kind: 'stone'` do catálogo; a lista Shiny é curada e validada separadamente.
- Antes de `sellAllLoot`, chama `toggleBagLock(itemId)` apenas para itens possuídos presentes em `protectedItemIds` e ainda ausentes de `hud.bagLocks`. Bloqueios oficiais existentes nunca são removidos automaticamente.
- A venda só pode ser enfileirada depois que todos os novos IDs aparecem em `hud.bagLocks`, com uma nova consulta às proteções imediatamente antes de vender. Falha, action ausente ou timeout de confirmação cancela apenas a venda; as compras continuam pela fila oficial com o saldo disponível. Falha ao enviar a venda também não bloqueia compras, mas nunca repita uma venda incerta no mesmo ciclo. O resultado mantém `saleStatus` e `saleWarning`.
- Cada chamada só conta como enfileirada quando a action correspondente aparece no `actionLog` com `seq` novo. Actions nativas podem intercalar-se durante a espera pelos locks: não exija seq consecutivo entre a proteção e o refill. Falha ao comprar mantém as categorias afetadas desarmadas; não acrescente retry próprio por fora da fila oficial. Após estoque recuperado, o próximo ciclo consulta novamente os locks e tenta somente os itens ainda desbloqueados; uma falha de proteção não desarma categorias cujas compras funcionaram.
- Após enfileirar compras, a UI espera observar no inventário o estoque anterior mais a quantidade de cada produto. Cada observação é retida mesmo se o produto voltar a ser consumido. Ao observar todos, volta a monitorar e limpa o marcador de ciclo pendente. Após 30 segundos sem observação completa, apenas avisa; não envia retry. Isso confirma estoque observado no store, não um ACK adicional do servidor. Pausar/desinstalar limpa esse acompanhamento e seu timer.
